 ...
 *
 * Режим «листовой материал»: сегменты раскладки листа — не в «кармане» SIP между OSB,
 * а на полную толщину стены (иначе в плане остаётся полоса как у EPS/ядра).
 */
export function boardCoreNormalOffsetsMm(
  ...
  if (profile) {
    if (resolveWallCalculationModel(profile) === "sheet") {
      return { offStartMm: -T / 2, offEndMm: T / 2 };
    }
