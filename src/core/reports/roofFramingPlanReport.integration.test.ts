...
describe("roof_framing_plan", () => {
  it("регистрируется и без стропильной системы даёт blocked", () => {
    const def = getReportDefinition("roof_framing_plan");
    expect(def?.viewKind).toBe("roof_framing_plan");
    ...
    expect(model.messages.some((m) => m.includes("стропильн") || m.includes("Нет геометрии"))).toBe(true);
  });

  it("с одной стропилиной собирает примитивы", () => {
    ...
    expect(model.messages.some((m) => m.startsWith("На листе:"))).toBe(true);
  });
});
